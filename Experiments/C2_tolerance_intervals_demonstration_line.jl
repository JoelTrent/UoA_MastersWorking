...
θ_true = [3,2,1]
true_dist = Normal(0, θ_true[3])
n = 101
t=LinRange(0, 10, n)
y_true = θ_true[1] .* t .+ θ_true[2] 

Random.seed!(3)
y_obs = y_true .+ rand(true_dist, n) 
...
#     return sum(loglikelihood(Normal(0, θ[3]), data.y_obs .- θ[1]))
# end

# function predictfunction(θ, data, t=data.t)
#     return θ[1] .* t .+ θ[2]
# end

# errorfunction(a,b,c) = normal_error_σ_estimated(a,b,c, 3)
