...
if !isfile(joinpath(output_location, "confidence_interval_ll_calls_ellipseapprox_start_simultaneous_threshold.csv"))
    ...
    function record_CI_LL_evaluations!(timer_df, N)
        ...
        for j in 1:N
            ...
            opt_settings = create_OptimizationSettings(solve_kwargs=(maxtime=5, xtol_rel=1e-12))
            for i in 1:model.core.num_pars
                univariate_confidenceintervals!(model, [i], profile_type=EllipseApproxAnalytical(), dof=model.core.num_pars, existing_profiles=:overwrite)

                TO.reset_timer!(LikelihoodBasedProfileWiseAnalysis.timer)
                univariate_confidenceintervals!(model, [i], use_ellipse_approx_analytical_start=true, existing_profiles=:overwrite, dof=model.core.num_pars, optimizationsettings=opt_settings)
...
if !isfile(joinpath(output_location, "confidence_boundary_ll_calls_simultaneous_threshold_10points_xtol_rel.csv"))
    ...
    function record_CI_LL_evaluations!(timer_df, N)
        ...
        for j in 1:N 
            ...
            for (i, pars) in enumerate(collect(combinations(1:model.core.num_pars, 2)))
                TO.reset_timer!(LikelihoodBasedProfileWiseAnalysis.timer)
                bivariate_confidenceprofiles!(model, [pars], 10, method=RadialMLEMethod(0.15, 0.01), existing_profiles=:overwrite, 
                    use_distributed=false, use_threads=false, optimizationsettings=opt_settings, dof=model.core.num_pars)
...
#########################################################################################################################
#########################################################################################################################
if !isfile(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_10points_xtol_rel.csv"))
    using Combinatorics
    ...
    for num_points in num_points_iter
        Random.seed!(1234)
        new_df = check_bivariate_prediction_coverage(data_generator, training_gen_args, t_pred, model, 1000, 10, θ_true, collect(combinations(1:model.core.num_pars, 2)),
            method=RadialMLEMethod(0.15, 0.01),
            num_internal_points=num_points,
            show_progress=true, distributed_over_parameters=false,
            dof=model.core.num_pars,
        ...
        new_df.num_points .= num_points
        global coverage_df = vcat(coverage_df, new_df)
        CSV.write(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_10points_xtol_rel.csv"), coverage_df)
        Arrow.write(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_10points_xtol_rel.arrow"), coverage_df)
        @everywhere GC.gc()
    end
end
...
if !isfile(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_four_combinations_10points_xtol_rel.csv"))
    using Combinatorics
    ...
    for num_points in num_points_iter
        Random.seed!(1234)
        new_df = check_bivariate_prediction_coverage(data_generator, training_gen_args, t_pred, model, 1000, 10, θ_true, [[1, 4], [1, 5], [2, 3], [2, 6]],
            method=RadialMLEMethod(0.15, 0.01),
            num_internal_points=num_points,
            show_progress=true, distributed_over_parameters=false,
            dof=model.core.num_pars, manual_GC_calls=true,
        ...
        new_df.num_points .= num_points
        global coverage_df = vcat(coverage_df, new_df)
        CSV.write(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_four_combinations_10points_xtol_rel.csv"), coverage_df)
        Arrow.write(joinpath(output_location, "bivariate_prediction_coverage_simultaneous_threshold_four_combinations_10points_xtol_rel.arrow"), coverage_df)
        @everywhere GC.gc()
    end
end
...
if !isfile(joinpath(output_location, "bivariate_realisation_coverage_simultaneous_threshold_10points_xtol_rel.csv"))
    using Combinatorics
    ...
    for num_points in num_points_iter
        Random.seed!(1234)
        new_df = check_bivariate_prediction_realisations_coverage(data_generator, reference_set_generator, training_gen_args, testing_gen_args, t_pred, model, 1000, 10, θ_true, collect(combinations(1:model.core.num_pars, 2)),
            method=RadialMLEMethod(0.15, 0.01),
            num_internal_points=num_points,
            show_progress=true, distributed_over_parameters=false,
            dof=model.core.num_pars,
...
if !isfile(joinpath(output_location, "bivariate_realisation_coverage_simultaneous_threshold_five_combinations_10points_xtol_rel.csv"))
    using Combinatorics
    ...
    for num_points in num_points_iter
        Random.seed!(1234)
        new_df = check_bivariate_prediction_realisations_coverage(data_generator, reference_set_generator, training_gen_args, testing_gen_args, t_pred, model, 1000, 10, θ_true, [[1, 4], [1, 5], [2, 3], [2, 6], [2, 7]],
            method=RadialMLEMethod(0.15, 0.01),
            num_internal_points=num_points,
            show_progress=true, distributed_over_parameters=false,
            manual_GC_calls=true,
        ...
        new_df.num_points .= num_points
        global coverage_df = vcat(coverage_df, new_df)
        CSV.write(joinpath(output_location, "bivariate_realisation_coverage_simultaneous_threshold_five_combinations_10points_xtol_rel.csv"), coverage_df)
        Arrow.write(joinpath(output_location, "bivariate_realisation_coverage_simultaneous_threshold_five_combinations_10points_xtol_rel.arrow"), coverage_df)
        @everywhere GC.gc()
    end
end
