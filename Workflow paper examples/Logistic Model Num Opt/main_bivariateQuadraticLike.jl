...
CtraceQuad = reduce(hcat, [model(tt,[λs[i],Ks[i],C0s[i]],σ) for i in 1:N if ellipse_loglike(λs[i],Ks[i],C0s[i]) > llstar])

CUQuad = maximum(CtraceQuad, dims=2)
CLQuad = minimum(CtraceQuad, dims=2)

qq1 = plotPredictionComparison(tt, CtraceF, (CUF, CLF), (CUQuad, CLQuad), 
...
function bivariateλK(λ,K)
    function funλK(a); return ellipse_loglike(λ,K,a[1]) end
...
function bivariateλC0(λ,C0)
    function funλC0(a); return ellipse_loglike(λ,a[1],C0) end
...
function bivariateKC0(K,C0)
    function funKC0(a); return ellipse_loglike(a[1],K,C0) end
...
a3 = plot2DBoundaryComparison((Ksamples_boundary, C0samples_boundary), 
                            (Ksamples_boundary1, C0samples_boundary1), (Kmle, C0mle), N, 
