...
f(x) = univariateλ(x)[1]
M=100;
λrangeQuad=LinRange(λmin,λmax,M)
ffQuad=zeros(M)
for i in 1:M
    global ffQuad[i] = univariateλ(λrangeQuad[i])[1]
end

# q1 = plot1DProfile(λrange, exp.(ff), exp(llstar), λmle, xlims=(λmin,0.04), ylims=(0,1),
#                     xlabel="λ", ylabel="ll")
q1 = plot1DProfile(λrangeQuad, ffQuad, llstar, λmle, xlims=(λmin,0.04), ylims=(-3,0),
                    xlabel="λ", ylabel="ll")
display(q1)
...
f(x) = univariateK(x)[1]
M=100;
KrangeQuad=LinRange(Kmin,Kmax,M)
ffQuad=zeros(M)
for i in 1:M
    global ffQuad[i]=univariateK(KrangeQuad[i])[1]
end
...
q2 = plot1DProfileComparison(Krange, ff, KrangeQuad, ffQuad, llstar, λmle, xlims=(80,120), 
...
M=100;
C0rangeQuad=LinRange(0,C0max,M)
ffQuad=zeros(M)
for i in 1:M
    global ffQuad[i]=univariateC0(C0rangeQuad[i])[1]
end
